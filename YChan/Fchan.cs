...
namespace YChan {
    class Fchan : Imageboard {
        public static string regThread  = "boards.4(chan|channel).org/[a-zA-Z0-9]*?/thread/[0-9]*";
        public static string regBoard   = "boards.4(chan|channel).org/[a-zA-Z0-9]*?/$";
        ...
        public Fchan(string url, bool isBoard) : base(url, isBoard) {
            ...
            this.imName    = "4chan";
            if(!isBoard) {
                Match match = Regex.Match(url, @"boards.4(chan|channel).org/[a-zA-Z0-9]*?/thread/\d*");
